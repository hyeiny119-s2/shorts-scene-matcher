...
설치가 끝나면 **`run.bat`** 을 더블클릭해서 실행하세요.
...
> 출력 폴더는 `data/{영화파일명}_{날짜시간}` 형식으로 자동 생성됩니다.
...
완료되면 **📊 리포트 열기** 버튼을 클릭하면 브라우저에서 매칭 결과를 바로 확인할 수 있습니다.

출력 파일은 **📁 폴더** 버튼으로 열 수 있습니다.

| 파일/폴더 | 내용 |
|-----------|------|
| `{영화파일명}_final.mp4` | 전체 컷을 이어붙인 최종 영상 |
| `{영화파일명}_report.html` | 매칭 결과 리포트 (브라우저에서 열기) |
| `img/` | 리포트에 사용된 썸네일 이미지 모음 |
