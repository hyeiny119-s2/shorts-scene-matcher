...
설치가 끝나면 **바탕화면**에 **Shorts Auto Editor** 바로가기가 생깁니다.

더블클릭해서 실행하세요. 또는 폴더 안의 **`run.bat`** 을 더블클릭해도 됩니다.
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| **유사도 필터** | 체크 | 체크 시 유사도 0.4 이상만 매칭, 해제 시 0.1 이상 (더 많이 매칭) |
| **시간순 정렬** | 체크 | 같은 장면이 반복될 때 체크, 영화 시간 순서대로 정렬 |
| **컷 개별 저장** | 해제 | 체크 시 `clips/` 폴더에 컷별로 개별 파일 추가 저장 |
...
| 파일/폴더 | 내용 |
|-----------|------|
| `(이름)_final.mp4` | 전체 컷을 이어붙인 최종 영상 |
| `(이름)_report.html` | 매칭 결과 리포트 (브라우저에서 열기) |
| `clips/` | 컷 개별 저장 옵션 체크 시 생성, 컷별 mp4 파일 저장 |
